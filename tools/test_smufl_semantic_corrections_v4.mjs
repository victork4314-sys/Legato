...
const octaveAuditOld = "octaveClef: data.glyphs.find(g => g.kind === 'clef' && Number(g.semitones) === 12),";
const octaveAuditNew = "octaveClef: data.glyphs.find(g => g.kind === 'clef' && Number(g.semitones) === 12 && /^gClef/i.test(g.id || '')),";
if (source.split(octaveAuditOld).length !== 2) throw new Error('Could not restrict the octave-clef audit example to a G clef');
source = source.replace(octaveAuditOld, octaveAuditNew);
const octavePlaybackOld = "const octave = cat.find(g => g.kind === 'clef' && Number(g.semitones) === 12);";
const octavePlaybackNew = "const octave = cat.find(g => g.kind === 'clef' && Number(g.semitones) === 12 && /^gClef/i.test(g.id || ''));";
if (source.split(octavePlaybackOld).length !== 2) throw new Error('Could not restrict the octave-clef playback check to a G clef');
source = source.replace(octavePlaybackOld, octavePlaybackNew);
