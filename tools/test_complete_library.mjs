...
const catalogAudit = await page.evaluate(() => {
  ...
  const groups = Array.from(new Set(cat.glyphs.map(g => g.group)));
  ...
  return { glyphCount: cat.glyphCount, rangeCount: cat.rangeCount, groups, ranges: ranges.length, audible: audible.length, specialist: specialist.length, bad: bad.slice(0, 5) };
});
...
assert.ok(catalogAudit.groups.includes('Popular'), 'Popular must be a first-class catalog group');
assert.ok(catalogAudit.groups.includes('Specialist & optional'), 'specialist and optional notation must be separated but present');
...
const visualResult = await page.evaluate(() => {
  ...
  owner.setState({ selId: null, halo: true }, () => owner.applyCatalogCommand(item, item.label, item.glyph));
...
const microResult = await page.evaluate(() => {
  ...
  owner.setState({ selId: 'n-main', scoreObjectId: null }, () => owner.applyCatalogCommand(item, item.label, item.glyph));
...
const articulationProfile = await page.evaluate(() => {
  ...
  owner.setState({ selId: 'n-main' }, () => owner.applyCatalogCommand(item, item.label, item.glyph));
...
const techniqueResult = await page.evaluate(() => {
  ...
  owner.setState({ selId: null, pos: 1 }, () => owner.applyCatalogCommand(item, item.label, item.glyph));
...
const hairpinResult = await page.evaluate(() => {
  const owner = window.__legatoOwner, cat = window.LEGATO_SMUFL_CATALOG;
  const item = cat.glyphs.find(g => g.kind === 'hairpin' && g.direction === 'up' && g.glyph);
  owner.setState({ selId: null, pos: 0.5, staff: 0 }, () => owner.applyCatalogCommand(item, item.label, item.glyph));
  return { id: item.id };
});
await pause(100);
...
const selectorKinds = await page.evaluate(() => {
  const owner = window.__legatoOwner;
  owner.setState(s => ({ notes: s.notes.concat([{ id: 'r-one', s: 0, p: 2, d: 'q', step: 6, voice: 1, rest: true }]) }));
...
const chordMove = await page.evaluate(() => {
  const owner = window.__legatoOwner;
