...
replace_once("    if (/muted/.test(tech) && /guitar/.test(base)) return 'electric_guitar_muted';\n    const percussion", "    if (/muted/.test(tech) && /guitar/.test(base)) return 'electric_guitar_muted';\n    if (/col-legno|chop|click|snap|slap/.test(tech)) return 'woodblock';\n    if (/behind-bridge|scrape/.test(tech)) return 'reverse_cymbal';\n    if (/sul-ponticello/.test(tech) && /violin|viola|cello|contrabass|string|fiddle/.test(base)) return 'tremolo_strings';\n    if (/sul-tasto/.test(tech) && /violin|viola|cello|contrabass|string|fiddle/.test(base)) return 'string_ensemble_1';\n    if (/flutter|growl/.test(tech)) return 'muted_trumpet';\n    if (/air/.test(tech)) return 'pan_flute';\n    const percussion", 'extended technique sample routing')
replace_once("    if (/bell|triangle/.test(perc)) return 'tinkle_bell';\n    return base;", "    if (/handbell|bell|triangle/.test(perc)) return 'tinkle_bell';\n    if (/siren|whistle/.test(perc)) return 'whistle';\n    if (/clap|click|snap|slap|stamp|tap|hop/.test(perc)) return 'woodblock';\n    if (/tambourine|shaker|maraca/.test(perc)) return 'agogo';\n    return base;", 'extended percussion sample routing')
replace_once("      const dynEv = this.effectiveScoreEvent('dynamic', n.s, n.p, s), dynGlyph = n.dyn || (dynEv && (dynEv.glyph || dynEv.value));\n      let vel = Math.round(dynEv && dynEv.playback && dynEv.playback.velocity != null ? dynEv.playback.velocity : (dynGlyph && dynMap[dynGlyph] ? dynMap[dynGlyph] : 40 + (s.vel / 100) * 87));", "      const dynEv = this.effectiveScoreEvent('dynamic', n.s, n.p, s), dynGlyph = n.dyn || (dynEv && (dynEv.glyph || dynEv.value));\n      let vel = Math.round(dynEv && dynEv.playback && dynEv.playback.velocity != null ? dynEv.playback.velocity : (dynGlyph && dynMap[dynGlyph] ? dynMap[dynGlyph] : 40 + (s.vel / 100) * 87));\n      const electronic = this.activeElectronic(n.p, s);\n      if (electronic && electronic.playback) {\n        if (electronic.playback.electronic === 'mute') return;\n        if (electronic.playback.electronic === 'fade') vel = Math.round(vel * .58);\n      }", 'electronic playback effect')
replace_once("      const ap = this.articulationPlayback(n);\n      const originalInstrument = s.instruments[n.s], semanticInstrument = this.techniqueInstrument(n.s, n.p, n, s);", "      const ap = this.articulationPlayback(n), tp = this.techniquePlaybackProfile(n.s, n.p, n, s);\n      const originalInstrument = s.instruments[n.s], semanticInstrument = this.techniqueInstrument(n.s, n.p, n, s);", 'technique playback profile use')
replace_once("      this.playTone(m, Math.max(ac.currentTime, when), duration * (decorated ? .18 : ap.length), n.s, (decorated ? vel * .8 : vel) * ap.gain, n.art);", "      this.playTone(m, Math.max(ac.currentTime, when), duration * (decorated ? .18 : ap.length * tp.length), n.s, (decorated ? vel * .8 : vel) * ap.gain * tp.gain, n.art);", 'technique duration and gain')
replace_once("      else if (/trill|shake/.test(pattern)) trill(/shake/.test(pattern) ? upStep + 1 : upStep, tN, tStep);\n      else if (/tremolo/.test(pattern)", "      else if (/trill|shake/.test(pattern)) trill(/shake/.test(pattern) ? upStep + 1 : upStep, tN, tStep);\n      else if (/acciaccatura/.test(pattern)) this.playTone(m + (semantic.direction === 'down' ? dnStep : upStep), Math.max(this.audio().currentTime + .002, when - .055), .065, staff, v * .82, null);\n      else if (/appoggiatura|grace/.test(pattern)) this.playTone(m + (semantic.direction === 'down' ? dnStep : upStep), when, Math.max(.08, dur * .38), staff, v * .88, null);\n      else if (/tremolo/.test(pattern)", 'grace-note playback patterns')
replace_once("        else if (ev.type === 'glyph' || ev.type === 'meter-glyph') { text2 = ev.glyph || ev.text || ev.name; family = 'Bravura,\\'Noto Music\\',serif'; size = ev.type === 'meter-glyph' ? 31 : 27; top = ev.type === 'meter-glyph' ? st.top + 3 : st.top - 28; extra = ''; }", "        else if (ev.type === 'glyph' || ev.type === 'meter-glyph' || ev.type === 'electronic') { text2 = ev.glyph || ev.text || ev.name; family = 'Bravura,\\'Noto Music\\',serif'; size = ev.type === 'meter-glyph' ? 31 : 27; top = ev.type === 'meter-glyph' ? st.top + 3 : st.top - 28; extra = ''; }", 'electronic glyph rendering')
required = ['20260727-complete-smufl-playback-2', "meta.direction === 'swell' ? 'hairpin-swell'", "if (meta.kind === 'electronic')", "meta.kind === 'ornament' || meta.kind === 'grace'", 'activeElectronic(pos, state)', 'techniquePlaybackProfile(staff, pos, note, state)', "electronic.playback.electronic === 'mute'", '/acciaccatura/.test(pattern)', "button4: 'previous-zone', button5: 'next-zone'", 'const BUMPER_COMBO_MS = 120;']
